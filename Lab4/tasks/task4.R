...
LOOCV = function(seed) {
    ...
    print(paste("seed --- ", seed))
      # 4.3.1
    fit.glm = glm(y ~ x)
    print(paste("LOOCV для тестової помилки [beta0 - beta1]: ",
    round(cv.glm(Coords, fit.glm)$delta[1], 2), "%"))

      # 4.3.2
    fit.glm2 = glm(y ~ x + I(x^2))
    print(paste("LOOCV для тестової помилки [beta0 - beta2]: ",
    round(cv.glm(Coords, fit.glm2)$delta[1], 2), "%"))

      # 4.3.3
    fit.glm3 = glm(y ~ poly(x, 3))
    print(paste("LOOCV для тестової помилки [beta0 - beta3]: ",
    round(cv.glm(Coords, fit.glm3)$delta[1], 2), "%"))

      # 4.3.4
    fit.glm4 = glm(y ~ poly(x, 4))
    print(paste("LOOCV для тестової помилки [beta0 - beta4]: ",
